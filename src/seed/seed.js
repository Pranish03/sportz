...
async function fetchMatches(limit = 100) {
    ...
    console.warn(
        "⚠️  GET /matches response had no recognizable array field (expected data/events/matches):",
...
async function createMatch(seedMatch) {
    ...
    // NOTE: This API wraps the created row under "event", not "data".
    // (Confirmed via: {"event":{"id":4,"sport":"football",...}})
    if (!responsePayload?.event) {
        throw new Error(
            `POST /matches responded ${response.status} but had no 'event' field: ${JSON.stringify(responsePayload)}`,
        );
    }
...
async function insertCommentary(matchId, entry) {
    ...
    const responsePayload = await response.json();
    // NOTE: If your commentary POST endpoint also wraps its response under
    // "event" instead of "data" (matching the /matches endpoint), swap the
    // line below to: return responsePayload.event ?? responsePayload.data ?? responsePayload.message;
    return responsePayload.data ?? responsePayload.event ?? responsePayload;
}
...
async function seed() {
    ...
    for (let i = 0; i < randomizedFeed.length; i += 1) {
        ...
        console.log(`📣 [Match ${match.id}] ${row?.message ?? entry.message}`);
